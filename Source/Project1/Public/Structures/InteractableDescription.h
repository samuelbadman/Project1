...
struct FInteractableDescription
{
	...
	// 0 - 1 and sampled values are multiplied with TapInteractionPercentCompleteIncrementAmount. Time values outside the 0 - 1 range are not sampled. The curve only gets sampled
	// when the interact input is started (pressed) For example, the curve would usually contain a point starting at 0, 1 and an ending point at 1, X. X is the final value 
	// multiplied with TapInteractionPercentCompleteIncrementAmount as the interaction completion percent increases. Points in between these two describe how the multiplier gets 
	// to the final value X. Only used when interaction type is set to tap
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "InteractableDescription")
